...
class BEVRender(BaseRender):
    ...
    def render_anno_data(self, sample_token, nusc, predict_helper):
    ...
    def show_lidar_data(self, sample_token, nusc):
        sample_record = nusc.get("sample", sample_token)
        assert (
            "LIDAR_TOP" in sample_record["data"].keys()
        ), "Error: No LIDAR_TOP in data, unable to render."
    ...
    def render_pred_map_data(self, predicted_map_seg):
        ...
        map_color_dict = np.array([(204, 128, 0), (102, 255, 102), (102, 255, 102)])
        rendered_map = map_color_dict[predicted_map_seg.argmax(-1).reshape(-1)].reshape(
            200, 200, -1
        )
    ...
    def render_occ_map_data(self, agent_list):
        ...
        for i in range(len(agent_list)):
            ...
            rendered_map_hsv[pred_occ_map > 0.1] = (np.ones((200, 200, 1)) * hsv_c)[
                pred_occ_map > 0.1
            ]
        ...
        self.axes.imshow(
            rendered_map, alpha=0.8, interpolation="nearest", extent=(-50, 50, -50, 50)
        )
    ...
    def render_hd_map(self, nusc, nusc_maps, sample_token):
        ...
        map_show[map_mask[1], :] = np.array([159.0 / 255.0, 0.0, 1.0])
        self.axes.imshow(
            map_show,
            alpha=0.2,
            interpolation="nearest",
