        ...
        for k in range(len(outputs)):
            ...
            if self.show_sdc_traj:
                outputs[k]["boxes_3d"].tensor = torch.cat(
                    [outputs[k]["boxes_3d"].tensor, outputs[k]["sdc_boxes_3d"].tensor],
                    dim=0,
                )
            ...
            if self.with_map:
                map_thres = 0.7
                score_list = (
                    outputs[k]["pts_bbox"]["score_list"]
                    .cpu()
                    .numpy()
                    .transpose([1, 2, 0])
                )
                predicted_map_seg = (
                    outputs[k]["pts_bbox"]["lane_score"]
                    .cpu()
                    .numpy()
                    .transpose([1, 2, 0])
                )  # H, W, C
            ...
            if self.with_planning:
                ...
                outputs[k]["planning_traj"][0][:, 0] = -outputs[k]["planning_traj"][0][
                    :, 0
                ]
...
def main(args):
    ...
    for i in range(len(viser.nusc.sample)):
        ...
        viser.visualize_bev(
            sample_token, os.path.join(args.out_folder, str(i).zfill(3))
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--predroot",
        default="/mnt/nas20/yihan01.hu/tmp/results.pkl",
        help="Path to results.pkl",
    )
    parser.add_argument(
        "--out_folder",
        default="/mnt/nas20/yihan01.hu/tmp/viz/demo_test/",
        help="Output folder path",
    )
    parser.add_argument(
        "--demo_video", default="mini_val_final.avi", help="Demo video name"
    )
    parser.add_argument(
        "--project_to_cam", default=True, help="Project to cam (default: True)"
    )
