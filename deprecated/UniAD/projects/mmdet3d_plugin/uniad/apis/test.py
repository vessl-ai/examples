...
def custom_multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False):
    ...
    # Occ eval init
    eval_occ = hasattr(model.module, "with_occ_head") and model.module.with_occ_head
    if eval_occ:
        ...
        for key in EVALUATION_RANGES.keys():
            panoptic_metrics[key] = PanopticMetric(
                n_classes=n_classes, temporally_consistent=True
            ).cuda()

    # Plan eval init
    eval_planning = (
        hasattr(model.module, "with_planning_head") and model.module.with_planning_head
    )
    if eval_planning:
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            if eval_planning:
                ...
                sdc_planning_mask = result[0]["planning"]["planning_gt"][
                    "sdc_planning_mask"
                ]
                pred_sdc_traj = result[0]["planning"]["result_planning"]["sdc_traj"]
                result[0]["planning_traj"] = result[0]["planning"]["result_planning"][
                    "sdc_traj"
                ]
                result[0]["planning_traj_gt"] = result[0]["planning"]["planning_gt"][
                    "sdc_planning"
                ]
                result[0]["command"] = result[0]["planning"]["planning_gt"]["command"]
                planning_metrics(
                    pred_sdc_traj[:, :6, :2],
                    sdc_planning[0][0, :, :6, :2],
                    sdc_planning_mask[0][0, :, :6, :2],
                    segmentation[0][:, [1, 2, 3, 4, 5, 6]],
                )
                ...
                if occ_to_eval and "occ" in result[0].keys():
                    ...
                    for key, grid in EVALUATION_RANGES.items():
                        ...
                        panoptic_metrics[key](
                            result[0]["occ"]["ins_seg_out"][..., limits, limits]
                            .contiguous()
                            .detach(),
            ...
            if os.environ.get("ENABLE_PLOT_MODE", None) is None:
                ...
                for k in [
                    ...
                    if k in result[0]["pts_bbox"] and isinstance(
                        result[0]["pts_bbox"][k], torch.Tensor
                    ):
                        result[0]["pts_bbox"][k] = (
                            result[0]["pts_bbox"][k].detach().cpu()
                        )
    ...
    if eval_occ:
        ...
        for key, grid in EVALUATION_RANGES.items():
            ...
            for panoptic_key, value in panoptic_scores.items():
                occ_results[f"{panoptic_key}"] = occ_results.get(
                    f"{panoptic_key}", []
                ) + [100 * value[1].item()]
        ...
        occ_results["ratio_occ"] = num_occ / len(
            dataset
        )  # count on one gpu, but reflect the relative ratio
