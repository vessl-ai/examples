...
class BevFeatureSlicer(nn.Module):
    def __init__(self, grid_conf, map_grid_conf):
        ...
        if grid_conf == map_grid_conf:
            ...
            tmp_m, tmp_n = torch.meshgrid(
                self.norm_map_x, self.norm_map_y
            )  # indexing 'ij'
    ...
    def forward(self, x):
        ...
        if self.identity_mapping:
            ...
            grid = (
                self.map_grid.unsqueeze(0).type_as(x).repeat(x.shape[0], 1, 1, 1)
            )  # (b, h, w, 2)
...
class Interpolate(nn.Module):
    ...
    def forward(self, x):
        return self._interpolate(
            x, scale_factor=self._scale_factor, mode="bilinear", align_corners=False
        )
