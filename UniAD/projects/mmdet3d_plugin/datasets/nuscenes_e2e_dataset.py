...
class NuScenesE2EDataset(NuScenesDataset):
        ...
        **kwargs,
        ...
        self.nusc_maps = {
            "boston-seaport": NuScenesMap(
                dataroot=self.data_root, map_name="boston-seaport"
            ),
            "singapore-hollandvillage": NuScenesMap(
                dataroot=self.data_root, map_name="singapore-hollandvillage"
            ),
            "singapore-onenorth": NuScenesMap(
                dataroot=self.data_root, map_name="singapore-onenorth"
            ),
            "singapore-queenstown": NuScenesMap(
                dataroot=self.data_root, map_name="singapore-queenstown"
            ),
        }
        self.vector_map = VectorizedLocalMap(
            self.data_root, patch_size=self.patch_size, canvas_size=self.canvas_size
        )
    ...
    def get_data_info(self, index):
        ...
        if "sdc_planning" in input_dict["ann_info"].keys():
            ...
            input_dict["sdc_planning_mask"] = input_dict["ann_info"][
                "sdc_planning_mask"
            ]
        ...
        input_dict["occ_future_ann_infos"] = self.get_future_detection_infos(
            future_frames
        )
        return input_dict
