...
class MotionHead(BaseMotionHead):
    ...
    def forward_train(
        ...
        sdc_match_index = torch.zeros(
            (1,), dtype=all_matched_idxes[0].dtype, device=all_matched_idxes[0].device
        )
        ...
        def filter_vehicle_query(
            outs_motion, all_matched_idxes, gt_labels_3d, vehicle_id_list
        ):
        ...
        outs_motion["sdc_traj_query"] = outs_motion["traj_query"][
            :, :, -1
        ]  # [3, 1, 6, 256]     [n_dec, b, n_mode, d]
        outs_motion["sdc_track_query"] = outs_motion["track_query"][
            :, -1
        ]  # [1, 256]           [b, d]
        outs_motion["sdc_track_query_pos"] = outs_motion["track_query_pos"][
            :, -1
        ]  # [1, 256]           [b, d]
        outs_motion["traj_query"] = outs_motion["traj_query"][
            :, :, :-1
        ]  # [3, 1, 3, 6, 256]  [n_dec, b, nq, n_mode, d]
    ...
    def forward(
        ...
        learnable_query_pos = self.learnable_motion_query_embedding.weight.to(
            dtype
        )  # latent anchor (P*G, D)
        ...
        for i in range(num_imgs):
            ...
            if self.use_nonlinear_optimizer:
                ...
                matched_gt_fut_traj, matched_gt_fut_traj_mask = nonlinear_smoother(
                    matched_gt_bboxes_3d,
                    matched_gt_fut_traj,
                    matched_gt_fut_traj_mask,
                    bboxes,
                )
