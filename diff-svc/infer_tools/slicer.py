...
def _window_maximum(arr, win_sz):
    return maximum_filter1d(arr, size=win_sz)[
        win_sz // 2 : win_sz // 2 + arr.shape[0] - win_sz + 1
    ]


def _window_rms(arr, win_sz):
    filtered = np.sqrt(
        uniform_filter1d(np.power(arr, 2), win_sz)
        - np.power(uniform_filter1d(arr, win_sz), 2)
    )
...
class Slicer:
    def __init__(
        ...
        if not self.min_samples >= self.win_ln >= self.win_sn:
            raise ValueError(
                "The following condition must be satisfied: min_length >= win_l >= win_s"
            )
        if not self.max_silence >= self.win_sn:
            raise ValueError(
                "The following condition must be satisfied: max_silence_kept >= win_s"
            )
    ...
    def slice(self, audio):
        ...
        while right < win_max_db.shape[0]:
            if win_max_db[right] < self.db_threshold:
                ...
                if left == 0:
                    ...
                    sil_left_n = min(
                        self.max_silence, (right + self.win_ln - left) // 2
                    )
                    rms_db_left = level2db(
                        _window_rms(
                            samples[left : left + sil_left_n], win_sz=self.win_sn
                        )
                    )
                    split_win_l = left + np.argmin(rms_db_left)
                    split_loc_l = split_win_l + np.argmin(
                        abs_amp[split_win_l : split_win_l + self.win_sn]
                    )
                if (
                    len(sil_tags) != 0
                ...
                if right == win_max_db.shape[0] - 1:
                    ...
                    rms_db_right = level2db(
                        _window_rms(
                            samples[
                                right + self.win_ln - sil_right_n : right + self.win_ln
                            ],
                            win_sz=self.win_sn,
                        )
                    )
                    split_win_r = (
                        right + self.win_ln - sil_right_n + np.argmin(rms_db_right)
                    )
                    split_loc_r = split_win_r + np.argmin(
                        abs_amp[split_win_r : split_win_r + self.win_sn]
                    )
        ...
        if left != right:
            ...
            rms_db_left = level2db(
                _window_rms(samples[left : left + sil_left_n], win_sz=self.win_sn)
            )
            split_win_l = left + np.argmin(rms_db_left)
            split_loc_l = split_win_l + np.argmin(
                abs_amp[split_win_l : split_win_l + self.win_sn]
            ...
            for i in range(0, len(sil_tags)):
                ...
                if i:
                    chunks.append(
                        {
                            "slice": False,
                            "split_time": f"{sil_tags[i - 1][1]},{sil_tags[i][0]}",
                        }
                    )
                # 标识所有静音片段
                chunks.append(
                    {"slice": True, "split_time": f"{sil_tags[i][0]},{sil_tags[i][1]}"}
                )
            # 最后一段静音并非结尾，补上结尾片段
            if sil_tags[-1][1] != len(audio):
                chunks.append(
                    {"slice": False, "split_time": f"{sil_tags[-1][1]},{len(audio)}"}
                )
            chunk_dict = {}
