    ...
    def module3(
        ...
        def add_pitch(decoder_inp, f0, uv, mel2ph, ret, encoder_out=None):
            if hparams["pitch_type"] == "ph":
                pitch_pred_inp = encoder_out.detach() + hparams["predictor_grad"] * (
                ...
                ret["f0_denorm"] = f0_denorm = denorm_f0(
                    f0, None, hparams, pitch_padding=pitch_padding
                )
            ...
            if energy is not None:
                ...
                if delta_l > 0:
                    energy = torch.cat(
                        (
                            energy,
                            torch.FloatTensor([[x[-1]] * delta_l for x in energy]).to(
                                energy.device
                            ),
                        ),
                        1,
    ...
    def insert4(
        ...
        *decoder_inp* ~= *decoder_inp* + embeddings for spk, pitch, energy
        ...
        ret["decoder_inp"] = decoder_inp = (
            decoder_inp + pitch_embedding + energy_embedding + spk_embed
        ) * tgt_nonpadding
        return decoder_inp

    @staticmethod
    def module4(
        diff_main_loss,  # modules
        norm_spec,
        decoder_inp_t,
        ret,
        K_step,
