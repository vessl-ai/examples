...
def export(p, state_dict, filepath="model.bin"):
    ...
    for i in range(p["n_layers"]):
        serialize(f"layers.{i}.attention_norm.weight")
    for i in range(p["n_layers"]):
        serialize(f"layers.{i}.attention.wq.weight")
    for i in range(p["n_layers"]):
        serialize(f"layers.{i}.attention.wk.weight")
    for i in range(p["n_layers"]):
        serialize(f"layers.{i}.attention.wv.weight")
    for i in range(p["n_layers"]):
        serialize(f"layers.{i}.attention.wo.weight")
    # ffn weights
    for i in range(p["n_layers"]):
        serialize(f"layers.{i}.ffn_norm.weight")
    for i in range(p["n_layers"]):
        serialize(f"layers.{i}.feed_forward.w1.weight")
    for i in range(p["n_layers"]):
        serialize(f"layers.{i}.feed_forward.w2.weight")
    for i in range(p["n_layers"]):
        serialize(f"layers.{i}.feed_forward.w3.weight")
