...
def load_model(model_name, quantization_method):
    ...
    if quantization_method == "awq":
        ...
        raise ValueError(
            f"Unsupported quantization method: {quantization_method}, supported methods are 'awq' and 'none'"
        )
...
def evaluate_model(model, tokenizer, streamer, prompts, max_length=2048):
    ...
    for prompt in prompts:
        ...
        generation_output = model.generate(
            tokens, streamer=streamer, max_new_tokens=512
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Name of the model to load from Hub",
    )
    parser.add_argument(
        "--prompts",
        type=str,
        nargs="+",
        required=True,
        help="Prompts to evaluate the model with.",
    )
    parser.add_argument(
        "--quantization", type=str, default="none", help="Quantization method to use."
    )
