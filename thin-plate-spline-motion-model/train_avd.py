...
def random_scale(kp_params, scale):
    ...
    new_kp_params = {
        "fg_kp": torch.matmul(theta, kp_params["fg_kp"].unsqueeze(-1)).squeeze(-1)
    }
...
def train_avd(
    ...
    optimizer = torch.optim.Adam(
        avd_network.parameters(), lr=train_params["lr"], betas=(0.5, 0.999)
    )
        ...
        for epoch in trange(start_epoch, train_params["num_epochs"]):
            ...
            for x in dataloader:
                with torch.no_grad():
                    ...
                    kp_driving_random = random_scale(
                        kp_driving_gt, scale=train_params["random_scale"]
                    )
                rec = avd_network(kp_source, kp_driving_random)

                reconstruction_kp = (
                    train_params["lambda_shift"]
            ...
            with torch.no_grad():
                ...
                driving = torch.cat(
                    [x["driving"][[0, 1]].cuda(), source[[2, 3, 2, 1]]], dim=0
                )
                ...
                dense_motion = dense_motion_network(
                    source_image=source, kp_driving=kp_driving, kp_source=kp_source
                )
