...
class Logger:
    ...
    def log_scores(self, loss_names, epoch):
        loss_mean = np.array(self.loss_list).mean(axis=0)

        for name, value in zip(loss_names, loss_mean):
            vessl.log(step=epoch, payload={name: value})
    ...
    def visualize_rec(self, inp, out, epoch):
    ...
    def log_epoch(self, epoch, models, inp, out):
        ...
        if (self.epoch + 1) % self.checkpoint_freq == 0:
            self.save_cpk()
        self.log_scores(self.names, epoch)
        self.visualize_rec(inp, out, epoch)
    ...
    def visualize(self, driving, source, out):
        ...
        if "deformed_source" in out:
            ...
            for i in range(out["deformed_source"].shape[1]):
                ...
                if i != 0:
                    color = np.array(
                        self.colormap((i - 1) / (out["deformed_source"].shape[1] - 1))
                    )[:3]
                else:
