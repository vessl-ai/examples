...
The app is now good to go. Mention the bot in a thread and ask it a question or use the `summarize` command for a summary, `--limit` to specify the number of messages (default: 10), `--help` command for instructions, and `--version` to check the app version.