...
def streamlit_display_title():
    ...
    st.title("VESSL AI: Manage your own Langchain session!")
    for e in intro:
...
def main():
    ...
    if "uploaded_files" not in st.session_state and "git_repo" not in st.session_state:
        ...
        chunk_overlap = st.number_input(
            "Enter chunk overlap [How much to overlap splitted texts]", value=100
        )
...
interactive:
  ...
  ports:
    ...
    st.markdown(
        f'<p style="font-family:Courier; color:Black; font-size: 20px;">For further details, visit <a href="https://vessl.ai/">this link!</a>',
        unsafe_allow_html=True,
    )
