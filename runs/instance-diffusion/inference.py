...
class Arguments:
    def __init__(self, output, num_images, guidance_scale, negative_prompt, input_json, ckpt, seed, alpha, mis, cascade_strength, test_config):
        self.output = output
...
@torch.no_grad()
def get_model_inputs(meta, model, text_encoder, diffusion, clip_model, clip_processor, config, grounding_tokenizer_input, starting_noise=None, instance_input=False, args: Arguments=None, return_att_masks=False):
...
@torch.no_grad()
def run(meta, model, autoencoder, text_encoder, diffusion, clip_model, clip_processor, config, grounding_tokenizer_input, starting_noise=None, guidance_scale=None, args: Arguments=None, return_att_masks=False):
    # prepare models inputs
    input, sampler, steps, uc, config = get_model_inputs(meta, model, text_encoder, diffusion, clip_model, clip_processor, config, grounding_tokenizer_input, starting_noise, instance_input=False, args=args, return_att_masks=return_att_masks)
    ...
    if args.mis > 0:
        ...
        for i in range(len(meta['phrases'])):
            ...
            input_instance, _, _, _, _ = get_model_inputs(meta_instance, model, text_encoder, diffusion, clip_model, clip_processor, config, grounding_tokenizer_input, starting_noise, instance_input=True, args=args, return_att_masks=return_att_masks)
...
if __name__ == "__main__":  
    ...
    for meta in meta_list:
        run(meta, model, autoencoder, text_encoder, diffusion, clip_model, clip_processor, config, grounding_tokenizer_input, starting_noise, guidance_scale=args.guidance_scale, args=args, return_att_masks=return_att_masks)
        


def infer_by_streamlit(output, num_images, guidance_scale, negative_prompt, input_json, ckpt, seed, alpha, mis, cascade_strength, test_config):
    args = Arguments(output, num_images, guidance_scale, negative_prompt, input_json, ckpt, seed, alpha, mis, cascade_strength, test_config)

    return_att_masks = False
    ckpt = args.ckpt

    seed = args.seed
    save_folder_name = f"output_images"
    ...
    for inst_idx in range(len(data['annos'])):
        ...
        if "bbox" not in data['annos'][inst_idx]: 
            ...
            boxes.append(data['annos'][inst_idx]['bbox'])
        if 'point' in data['annos'][inst_idx]:
            p = data['annos'][inst_idx]['point']
            if len(p) > 0:
                points_list.append(p)
        if "scribble" in data['annos'][inst_idx]:
            s = data['annos'][inst_idx]['scribble']
            if len(s) > 0:
                scribbles_list.append(p)
        # class_names.append(data['annos'][inst_idx]['category_name'])
        instance_captions.append(data['annos'][inst_idx]['caption'])
        # show_binary_mask(binay_masks[inst_idx])
