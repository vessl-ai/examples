...
def save_reviews(df: pd.DataFrame, output_path: str) -> str:
    ...
    for review in reviews:
        if "positive_features" in review:
            review["positive_features"] = parse_feature_list(
                review["positive_features"]
            )
        if "negative_features" in review:
            review["negative_features"] = parse_feature_list(
                review["negative_features"]
            )
...
def create_app(input_path: str, output_path: str):
    with gr.Blocks() as app:
        ...
        close_btn.click(fn=close_app, inputs=None, outputs=None)
...
def parse_arguments():
    ...
    parser.add_argument(
        "--input", "-i", type=str, required=True, help="Input JSON file path"
    )
    parser.add_argument(
