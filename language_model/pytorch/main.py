...
def train(model_type, model, corpus, train_data, batch_size, bptt, clip, log_interval, dry_run, epoch):
...
if __name__ == '__main__':
    ...
    # hyperparameters
    model_type = str(os.environ.get('model_type', 'LSTM'))   # RNN_TANH, RNN_RELU, LSTM, GRU, or Transformer
    ...
    dropout = float(os.environ.get('dropout', 0.2))          # dropout applied to layers (0 = no dropout)
    nhead = int(os.environ.get('nhead', 2))                  # the number of heads in the transformer model
    ...
    if model_type == 'Transformer':
        ...
        model = model.RNNModel(model_type, ntokens, emsize, nhid, nlayers, dropout, args.tied).to(device)
