...
def preprocess_original_data(input_path):
    ...
    df['scaled_time'] = robust_scaler.fit_transform(
...
if __name__ == '__main__':
    ...
    y = pd.read_csv(os.path.join(args.input_path, 'y.csv'), index_col=0,
    ...
    under_sample_fraud_predictions = np.argmax(
        under_sample_predictions,
        axis=1
    )

    under_sample_cm = confusion_matrix(
        original_y_test,
        under_sample_fraud_predictions
    )
    actual_cm = confusion_matrix(
        original_y_test,
        original_y_test
    )
