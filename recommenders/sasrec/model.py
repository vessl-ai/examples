...
class SASREC_Vessl(SASREC)  :
    ...
    def save_upload(self, save_path, epoch):
        #output_test/epoch_1.data-000... , json
...
class SSEPT_Vessl(SSEPT) :
    ...
    def train(self, dataset, sampler, **kwargs):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            self.save_upload(kwargs['save_path'], epoch)
            if epoch % val_epoch == 0 or epoch == num_epochs:
                t0.stop()
                t1 = t0.interval
                T += t1
                print("Evaluating...")
    ...
    def predict_next(self, input):
        ...
        for i in reversed(input):
            ...
            if idx == -1:
                break
        inputs = {}
        inputs["user"] = np.expand_dims(np.array(np.randint(1, self.user_num)), axis=0)  ## should be changed
        inputs["input_seq"] = np.array([seq])
        inputs["candidate"] = np.expand_dims(np.arange(1, self.item_num, 1), axis=0)
