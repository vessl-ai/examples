...
from recommenders.utils import gpu_utils, plot, tf_utils
...
if __name__ == "__main__":
    ...
    # Preprocess dataset
    new_data = (
        data.set_index(ITEM_COL)
        .join(movies.set_index(ITEM_COL))
        .filter([USER_COL, ITEM_COL, RATING_COL, ITEM_FEAT_COL])
        .reset_index()
    )
    ...
    items = new_data.drop_duplicates(ITEM_COL)[[ITEM_COL, ITEM_FEAT_COL]].reset_index(
        drop=True
    )
    ...
    model = wide_deep.build_model(
        ...
        linear_optimizer=tf_utils.build_optimizer(
            LINEAR_OPTIMIZER,
            LINEAR_OPTIMIZER_LR,
            **{
        ...
        dnn_optimizer=tf_utils.build_optimizer(
            DNN_OPTIMIZER,
            DNN_OPTIMIZER_LR,
            **{
            ...
            if len(metrics) > 0:
                hooks.append(
                    tf_utils.evaluation_log_hook(
                        ...
                        if metrics == RANKING_METRICS
                        else test.drop(RATING_COL, axis=1),
    ...
    print(
        "Training steps = {}, Batch size = {} (num epochs = {})".format(
            STEPS, BATCH_SIZE, (STEPS * BATCH_SIZE) // len(train)
        )
    )
    tf.compat.v1.logging.set_verbosity(tf.compat.v1.logging.INFO)

    try:
        model.train(input_fn=train_fn, hooks=hooks, steps=STEPS)
    ...
    if args.save_model:
        exported_path = tf_utils.export_model(
            ...
            eval_input_fn=tf_utils.pandas_input_fn(df=test, y_col=RATING_COL),
